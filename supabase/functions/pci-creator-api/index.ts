...
function corsHeaders(request: Request): HeadersInit {
  ...
  const allowedOrigin = origin && ALLOWED_ORIGINS.includes(origin)
    ? origin : ALLOWED_ORIGINS[0] ?? "http://localhost:5173";
  return {
...
function rpcErrorCode(error: { message?: string; code?: string } | null): { code: string; status: number } {
  ...
  const known = [
    "pci_creator_not_linked","pci_creator_not_active","pci_creator_workspace_access_denied","pci_consignment_not_found",
    "pci_consignment_not_open","pci_consignment_invitation_required","pci_active_participation_required","pci_submission_not_found",
    "pci_submission_limit_reached","pci_submission_version_not_found","pci_submission_version_not_allowed","pci_version_limit_reached",
    "pci_video_mime_not_allowed","pci_video_size_invalid","pci_sha256_invalid","pci_storage_object_not_verified",
    "pci_submission_version_mime_mismatch","pci_submission_version_not_finalizable","pci_submission_version_not_invalidatable",
    "pci_invalidation_reason_invalid","pci_ready_submission_version_immutable","pci_negotiation_message_context_required",
    "pci_negotiation_message_invalid","pci_negotiation_not_found","pci_negotiation_not_open","pci_offer_context_required",
    "pci_offer_not_found","pci_creator_can_only_reject_workspace_offer","pci_offer_not_live","pci_offer_expired",
    "pci_counter_requires_workspace_offer","pci_offer_amount_invalid","pci_counter_note_invalid","pci_offer_item_required",
    "pci_counter_multi_item_not_supported","pci_command_already_processing","pci_idempotency_conflict",
  ];
  ...
  if ([
    ...
    "pci_submission_version_not_invalidatable","pci_submission_version_not_allowed","pci_consignment_not_open",
    "pci_submission_limit_reached","pci_version_limit_reached","pci_ready_submission_version_immutable",
    "pci_negotiation_not_open","pci_creator_can_only_reject_workspace_offer","pci_offer_not_live","pci_offer_expired",
    "pci_counter_requires_workspace_offer","pci_offer_item_required","pci_counter_multi_item_not_supported",
  ].includes(code)) return { code, status: 409 };
  if ([
    "pci_video_mime_not_allowed","pci_video_size_invalid","pci_sha256_invalid","pci_submission_version_mime_mismatch",
    "pci_storage_object_not_verified","pci_invalidation_reason_invalid","pci_negotiation_message_invalid",
...
async function createUploadReservation(request: Request, admin: SupabaseAdmin, userId: string, submissionId: string, payload: JsonRecord): Promise<Response> {
  ...
  const command = await rpc(admin, "reserve_submission_version", {
    p_actor_user_id: userId, p_submission_id: submissionId, p_idempotency_key: idem,
    p_request_id: reqId, p_original_filename: originalFilename, p_mime_type: mimeType,
  });
  if (command.error) {
  ...
  const bucket = clean(result.storage_bucket), path = clean(result.storage_path);
  ...
  return json(request, {
    ...
    upload: {
      protocol: "tus", endpoint: DIRECT_STORAGE_TUS_ENDPOINT, bucket_name: bucket, object_name: path,
      content_type: mimeType, signature_token: signed.token, signature_header: "x-signature",
      chunk_size_bytes: 6 * 1024 * 1024, upsert: false, signed_token_ttl_seconds: 7200,
    },
  });
}
...
async function persistInvalidVersion(admin: SupabaseAdmin, userId: string, versionId: string, idem: string, reqId: string, reasonCode: "file_too_large" | "mime_mismatch" | "invalid_media" | "storage_metadata_invalid", validation: JsonRecord): Promise<void> {
  const result = await rpc(admin, "invalidate_submission_version", {
    p_actor_user_id: userId, p_submission_version_id: versionId, p_idempotency_key: idem,
    p_request_id: reqId, p_reason_code: reasonCode, p_validation_metadata: validation,
  });
...
async function finalizeVersion(request: Request, admin: SupabaseAdmin, userId: string, versionId: string, payload: JsonRecord): Promise<Response> {
  ...
  const bucket = clean(uploadContext.storage_bucket), path = clean(uploadContext.storage_path), expectedMime = clean(uploadContext.mime_type).toLowerCase();
  ...
  const validation: JsonRecord = {
    object_exists: true, verified_by: "pci-creator-api", storage_object_id: stored.objectId,
    storage_size_bytes: stored.size, storage_mime_type: stored.mimeType, storage_checked_at: new Date().toISOString(),
  };
  ...
  return rpcJson(request, admin, "finalize_submission_version", {
    p_actor_user_id: userId, p_submission_version_id: versionId, p_idempotency_key: idem, p_request_id: reqId,
    p_file_size_bytes: stored.size, p_mime_type: expectedMime, p_sha256: sha256,
    p_duration_seconds: Number.isFinite(duration as number) ? duration : null,
    p_width: Number.isInteger(width) ? width : null, p_height: Number.isInteger(height) ? height : null,
...
Deno.serve(async (request) => {
  ...
  if (request.method === "GET" && path === "/v1/opportunities") {
    const reqId = requestId(); return rpcJson(request, admin, "creator_opportunities", { p_actor_user_id: user.id }, reqId);
  }
  if (request.method === "GET" && path === "/v1/submissions") {
    const reqId = requestId(); return rpcJson(request, admin, "creator_submissions", { p_actor_user_id: user.id }, reqId);
  }
  if (request.method === "GET" && path === "/v1/negotiations") {
    const reqId = requestId(); return rpcJson(request, admin, "creator_negotiations", { p_actor_user_id: user.id }, reqId);
  }

  let match = path.match(/^\/v1\/submissions\/([0-9a-f-]+)$/i);
  if (request.method === "GET" && match) {
    const reqId = requestId(), submissionId = match[1].toLowerCase();
    if (!isUuid(submissionId)) return json(request, { ok: false, code: "invalid_submission_id", request_id: reqId }, 400);
    return rpcJson(request, admin, "creator_submission_detail", { p_actor_user_id: user.id, p_submission_id: submissionId }, reqId);
  }

  match = path.match(/^\/v1\/negotiations\/([0-9a-f-]+)$/i);
  if (request.method === "GET" && match) {
    const reqId = requestId(), negotiationId = match[1].toLowerCase();
    if (!isUuid(negotiationId)) return json(request, { ok: false, code: "invalid_negotiation_id", request_id: reqId }, 400);
    return rpcJson(request, admin, "creator_negotiation_detail", { p_actor_user_id: user.id, p_negotiation_id: negotiationId }, reqId);
  }

  match = path.match(/^\/v1\/consignments\/([0-9a-f-]+)\/join$/i);
  if (request.method === "POST" && match) {
    const reqId = requestId(), consignmentId = match[1].toLowerCase(), payload = await parseObject(request);
    ...
    if (!idem) return json(request, { ok: false, code: "idempotency_key_required", request_id: reqId }, 400);
    return rpcJson(request, admin, "join_consignment", { p_actor_user_id: user.id, p_consignment_id: consignmentId, p_idempotency_key: idem, p_request_id: reqId }, reqId);
  }

  if (request.method === "POST" && path === "/v1/submissions") {
    const reqId = requestId(), payload = await parseObject(request);
    if (!payload) return json(request, { ok: false, code: "invalid_json", request_id: reqId }, 400);
    const unexpected = rejectUnexpected(payload, ["consignment_id", "concept_label", "concept_metadata", "idempotency_key"]); if (unexpected.length) return json(request, { ok: false, code: "unexpected_fields", fields: unexpected, request_id: reqId }, 400);
    const consignmentId = clean(payload.consignment_id).toLowerCase(), idem = idempotencyKey(request, payload);
    if (!isUuid(consignmentId)) return json(request, { ok: false, code: "invalid_consignment_id", request_id: reqId }, 400);
    if (!idem) return json(request, { ok: false, code: "idempotency_key_required", request_id: reqId }, 400);
    const conceptMetadata = payload.concept_metadata && typeof payload.concept_metadata === "object" && !Array.isArray(payload.concept_metadata) ? payload.concept_metadata as JsonRecord : {};
    return rpcJson(request, admin, "create_submission", {
      p_actor_user_id: user.id, p_consignment_id: consignmentId, p_idempotency_key: idem, p_request_id: reqId,
      p_concept_label: clean(payload.concept_label) || null, p_concept_metadata: conceptMetadata,
    }, reqId, 201);
  }

  match = path.match(/^\/v1\/submissions\/([0-9a-f-]+)\/versions$/i);
  if (request.method === "POST" && match) {
    const submissionId = match[1].toLowerCase(); if (!isUuid(submissionId)) return json(request, { ok: false, code: "invalid_submission_id" }, 400);
    const payload = await parseObject(request); if (!payload) return json(request, { ok: false, code: "invalid_json" }, 400);
    return createUploadReservation(request, admin, user.id, submissionId, payload);
  }

  match = path.match(/^\/v1\/versions\/([0-9a-f-]+)\/finalize$/i);
  if (request.method === "POST" && match) {
    const versionId = match[1].toLowerCase(); if (!isUuid(versionId)) return json(request, { ok: false, code: "invalid_submission_version_id" }, 400);
    const payload = await parseObject(request); if (!payload) return json(request, { ok: false, code: "invalid_json" }, 400);
  ...
  if (request.method === "POST" && match) {
    const reqId = requestId(), negotiationId = match[1].toLowerCase(), payload = await parseObject(request);
    if (!isUuid(negotiationId)) return json(request, { ok: false, code: "invalid_negotiation_id", request_id: reqId }, 400);
    if (!payload) return json(request, { ok: false, code: "invalid_json", request_id: reqId }, 400);
    ...
    const body = clean(payload.body), idem = idempotencyKey(request, payload);
    if (!idem) return json(request, { ok: false, code: "idempotency_key_required", request_id: reqId }, 400);
    if (!body || body.length > 5000) return json(request, { ok: false, code: "invalid_message", request_id: reqId }, 400);
    return rpcJson(request, admin, "creator_send_negotiation_message", { p_actor_user_id: user.id, p_negotiation_id: negotiationId, p_body: body, p_idempotency_key: idem, p_request_id: reqId }, reqId, 201);
  }

  match = path.match(/^\/v1\/offers\/([0-9a-f-]+)\/(reject|counter)$/i);
  if (request.method === "POST" && match) {
    const reqId = requestId(), offerId = match[1].toLowerCase(), action = match[2].toLowerCase(), payload = await parseObject(request);
    if (!isUuid(offerId)) return json(request, { ok: false, code: "invalid_offer_id", request_id: reqId }, 400);
    if (!payload) return json(request, { ok: false, code: "invalid_json", request_id: reqId }, 400);
    const allowed = action === "counter" ? ["total_amount","counter_note","idempotency_key"] : ["idempotency_key"];
    ...
    if (action === "reject") return rpcJson(request, admin, "creator_reject_offer", { p_actor_user_id: user.id, p_offer_id: offerId, p_idempotency_key: idem, p_request_id: reqId }, reqId);
    ...
    return rpcJson(request, admin, "creator_counter_offer", { p_actor_user_id: user.id, p_parent_offer_id: offerId, p_total_amount: amount, p_counter_note: note || null, p_idempotency_key: idem, p_request_id: reqId }, reqId, 201);
  }
