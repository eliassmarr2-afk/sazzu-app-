-- Phase 1O runtime hardening.
-- New invitation deliveries bootstrap by authenticated Supabase identity + exact
-- invitation id. The raw PCI invitation token is no longer required in a
-- redirect URL and therefore must not be exposed to Auth/browser logs.
--
-- The legacy token bootstrap remains available to service_role for backwards
-- compatibility with already-issued flows, but new deliveries use this RPC.

create or replace function pci_api.creator_bootstrap_invitation_identity(
...
begin
  ...
  if v_invitation.invitation_id is null then
    raise exception using errcode='P0002', message='pci_creator_invitation_not_found';
  end if;
  ...
  if v_invitation.status = 'accepted' then
    ...
    if lower(v_invitation.email_snapshot) is distinct from v_email
       or lower(v_creator.email) is distinct from v_email then
  ...
  if v_invitation.delivery_status <> 'sent' then
    raise exception using errcode='23514', message='pci_creator_invitation_not_delivered';
  end if;
  if lower(v_invitation.email_snapshot) is distinct from v_email then
    raise exception using errcode='42501', message='pci_creator_invitation_email_mismatch';
  end if;
  if v_invitation.auth_user_id_snapshot is not null
     and v_invitation.auth_user_id_snapshot is distinct from p_actor_user_id then
    raise exception using errcode='42501', message='pci_creator_invitation_user_mismatch';
  end if;
  ...
  if v_creator.creator_id is null then
    raise exception using errcode='P0002', message='pci_creator_not_found';
  end if;
  if lower(v_creator.email) is distinct from v_email then
    raise exception using errcode='42501', message='pci_creator_invitation_email_mismatch';
  end if;
  if v_creator.auth_user_id is not null
     and v_creator.auth_user_id is distinct from p_actor_user_id then
  ...
  where wc.workspace_creator_id = v_invitation.workspace_creator_id
  for update;
  ...
  values(
    ...
    'creator_bootstrap_invitation_identity',
    p_request_id,
    'processing'
  )
  ...
  if v_receipt_id is null then
    ...
    where cr.actor_type = 'creator'
      ...
      and cr.command_name = 'creator_bootstrap_invitation_identity'
  ...
  set status = 'accepted',
      accepted_at = now(),
      auth_user_id_snapshot = p_actor_user_id
  ...
  perform pci.append_event(
    ...
    'creator_invitation.auth_bootstrapped',
    'pending',
    'accepted',
    p_request_id,
    v_receipt_id,
    jsonb_build_object(
      ...
      'bootstrap_method', 'authenticated_identity'
    )
  );
  ...
  v_result := jsonb_build_object(
    ...
    'workspace_creator_status', 'invited',
...
revoke all on function pci_api.creator_bootstrap_invitation_identity(uuid,text,uuid,uuid,uuid) from public, anon, authenticated;
grant execute on function pci_api.creator_bootstrap_invitation_identity(uuid,text,uuid,uuid,uuid) to service_role;

comment on function pci_api.creator_bootstrap_invitation_identity(uuid,text,uuid,uuid,uuid) is
'Creator onboarding bootstrap authorized by verified Auth identity, exact invitation id, delivery state, email snapshot and optional Auth user snapshot. No raw PCI invitation token is required.';
