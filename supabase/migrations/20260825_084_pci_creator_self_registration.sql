-- PCI 2.1S.1 · Creator spontaneous registration
-- Runtime-test first. Production must not be touched by this migration directly.

begin;

alter table pci.workspace_creators
  drop constraint if exists workspace_creators_status_check;

alter table pci.workspace_creators
  add constraint workspace_creators_status_check
  check (
    status = any (
      array[
        'pending'::text,
...
begin
  ...
  then
    raise exception
      using errcode = '22023',
      message = 'pci_workspace_creator_status_invalid';
  end if;
  ...
  then
    raise exception
      using errcode = '22023',
      message = 'pci_pagination_invalid';
  end if;
  ...
  from (
      ...
      jsonb_build_object(
        ...
        jsonb_build_object(
          ...
          (
            ...
            where
              n.workspace_id = p_workspace_id
              and n.creator_id = cr.creator_id
              and n.status = 'open'
          ),
          'purchases',
          (
            ...
            where
              ...
              and p.creator_id = cr.creator_id
          )
        ),
        'financial',
        jsonb_build_object(
          ...
          coalesce(
            (
              ...
              where
                py.workspace_id = p_workspace_id
                and py.creator_id = cr.creator_id
                and py.status in (
    ...
    join pci.creators cr
      on cr.creator_id = wc.creator_id
    where
      wc.workspace_id = p_workspace_id
      and (
        v_status is null
