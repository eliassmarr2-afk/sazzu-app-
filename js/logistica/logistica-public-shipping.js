...
(function () {
  ...
  function isFreeShipping(raw) {
    ...
    const cost = toNumber(raw.shipping_cost ?? raw.shippingCost ?? raw.envio_valor ?? raw.cost, 0);

    return raw.is_free === true || raw.isFree === true || label.includes('gratis') || status.includes('gratis') || cost === 0;
  }
  ...
  function normalizeLookupResponse(raw, requestedPostalCode) {
    ...
    const shippingCost = toNumber(source.shipping_cost ?? source.shippingCost ?? source.envio_valor ?? source.cost, 0);
    const isFree = ok ? isFreeShipping(source) : false;
    const promise = source.promise_label || source.promise || source.publicText || source.estimatedText || source.deliveryText || source.message || '';
    const minDays = source.min_days ?? source.minDays ?? source.minimum_days ?? null;
    const maxDays = source.max_days ?? source.maxDays ?? source.maximum_days ?? null;
    ...
    return {
      ...
      isFree,
      shippingCost,
      shippingLabel: isFree ? 'Envío gratis' : 'ENVIO RASTREABLE',
      ...
      province: source.province || source.provincia || null,
      zoneId: source.zone_id || source.zoneId || null,
      timeWindow: source.time_window || source.timeWindow || source.banda_horaria || null,
      banner: source.banner_id || source.bannerId || null,
