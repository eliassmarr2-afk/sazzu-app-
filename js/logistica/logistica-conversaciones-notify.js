/* ==========================================================
   ...
   Fase 5: botón manual QA, independiente del envío del mensaje.
   ========================================================== */

(function () {
  const PAGE_EVENT = 'sazzu:page:load';
  const READY_FLAG = '__protocolSupportReplyNotificationReady';
  const QA_CONVERSATION_ID = '21f6ca28-fad8-4eea-ab3e-c4dbe6dad698';
  ...
  function conversationId() {
    ...
    return String(form?.dataset.logConversationReplyReal || '').trim();
  }
  ...
  function injectButton() {
    ...
    if (!dataBox || id !== QA_CONVERSATION_ID) return;
    ...
    if (!wrapper) {
      ...
      wrapper.innerHTML = `
        <button class="logSupportReplyNotify__button" type="button" data-log-support-reply-notify="${QA_CONVERSATION_ID}">
          ${chatIcon()}
  ...
  function friendlyError(payload) {
    ...
    if (code === 'notification_already_sending') return 'El aviso ya se está procesando.';
  ...
  async function handleNotify(button) {
    const id = String(button?.dataset.logSupportReplyNotify || '').trim();
    if (!button || id !== QA_CONVERSATION_ID || id !== conversationId()) return;
    ...
    try {
      ...
      if (sessionResult.error || !sessionResult.data?.session) {
        throw new Error('missing_protocol_session');
      }
      ...
      if (data.sent === true) {
        setState(fingerprint, {
          ...
          label: 'Aviso enviado',
          kind: 'success',
          message: 'El cliente recibió un nuevo acceso seguro a esta conversación.'
        });
        return;
      }
