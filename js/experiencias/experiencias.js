...
(function () {
  ...
  function getRequestedTab_() {
    const params = new URLSearchParams(window.location.search);
    const requested = String(params.get("pdTab") || "").toLowerCase();
  ...
  function syncTabUrl_(target) {
    const url = new URL(window.location.href);

    if (target === "resumen") {
      url.searchParams.delete("pdTab");
    } else {
      url.searchParams.set("pdTab", target);
    }
  ...
  function syncSidebarShortcut_(target) {
    document
      .querySelectorAll(
        '[data-sidebar-shortcut-page="experiencias"]'
      )
      .forEach((shortcut) => {
        const tab = shortcut.getAttribute(
          "data-sidebar-shortcut-tab"
        );
  ...
  function init_(root) {
    ...
    const markupTab =
      root
        .querySelector("[data-exp-tab].is-active")
        ?.getAttribute("data-exp-tab") || "resumen";

    const initialTab = getRequestedTab_() || markupTab;
    ...
    root.addEventListener("click", (event) => {
      ...
      if (tab) {
        const target =
          tab.getAttribute("data-exp-tab") || "resumen";

        setActiveTab_(root, target);
        syncTabUrl_(target);
        return;
      }
