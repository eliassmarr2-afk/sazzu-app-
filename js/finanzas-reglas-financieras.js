...
(function () {
  const BUILD = "FINANCE_RULES_UI_2026_06_29_02";
  const VIEW_KEY = "sazzu_finanzas_active_view";
  const STYLE_ID = "finRulesStylesheet";
  const STYLE_HREF = "/css/finanzas-reglas-financieras.css?v=20260629_01";
    ...
    return `
      <div class="u-card finRulesCard">
        <div class="u-cardInner finRulesInner">
          ...
          <div class="finRulesLayout">
            <section class="finRulesEditor" aria-label="Editor de regla financiera">
              <div class="finRulesEditor__title">Editor de regla</div>
              ...
              <div class="u-muted" id="finRulesFormStatus" style="margin-top:10px;"></div>
            </section>
  ...
  function fillForm(rule) {
    ...
    state.selectedRuleCode = rule.rule_code || "";
  ...
  function renderRules() {
    ...
    body.innerHTML = state.rows.map(rule => {
      ...
      return `
        <tr data-fin-rule-code="${escapeHtml(rule.rule_code)}">
          <td>
  ...
  function wire() {
    ...
    if (fresh) fresh.addEventListener("click", clearForm);
    ...
    if (body) {
      body.addEventListener("click", ev => {
        ...
        if (!tr) return;
        const code = tr.getAttribute("data-fin-rule-code");
